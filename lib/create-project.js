...
const createNode = require('./server-node')
...
function confirmCreateServer(project) {
   ...
   inquirer.prompt(prompt).then((answers) => {
      ...
      } else if (answers.selection === 'Node/Express') {
         createNode(project)
      } else {
         quote('vader')
      }
...
async function confirmNewProject() {
   const prompt = {
      message:
         'This will create a new project directory in the current path.  What should the project be called?',
      name: 'project',
      type: 'input',
      default: 'my-project',
   }
