   ...
   try {
      ...
      spawn.sync(
         ...
         [
            'repo',
            'clone',
            'https://github.com/34fame/server-firebase-template',
            'server',
         ],
         {
            cwd: `./${project}`,
      ...
      fs.copyFileSync(`./vader/assets/.prettierrc`, `./${project}/server/.prettierrc`)
      fs.copyFileSync(`./vader/assets/.eslintrc`, `./${project}/server/.eslintrc`)
      fs.copyFileSync(`./vader/assets/jsconfig.json`, `./${project}/server/jsconfig.json`)

      console.log('Installing Firebase CLI tools...')
      // spawn.sync('npm', ['i', '-g', 'firebase-tools'])
      // TODO - Install gh ... requires brew on MacOS and choco on Windows
...
module.exports = async function (project) {
   ...
   console.log(
      chalk.bold.hex('#F39C12')(
         'You need to download the Server Account json file from your'
      )
   )
